function GameService() {
  ...
  let cities = ['London', 'Bangkok', 'Paris', 'Dubai', 'Istanbul', 'New York', 'Singapore', 'Kualal Lumpur', 'Seoul', 'Hong Kong', 'Tokyo', 'Barcelona', 'Amsterdam', 'Rome', 'Milan', 'Taipei', 'Vienna', 'Prague', 'Los Angeles']
  ...
  this.search = function (callWhenDone) {
    ...
    setTimeout(function () {
      ...
      if (gumShoe.city == carmen.city) {
        ...
        console.log(`A woman wearing a red hat and black boots is more likely found in ${getRandomCity()}`)
      }
  ...
  this.travel = function (index, callBack) {
    let cost = (((defaultTravelCost * index) + 1) + (cities.indexOf(gumShoe.city) + 1) / 10).toFixed()
    console.log(cost)
    if (gumShoe.budget - +cost > 0 && gumShoe.city != cities[index]) {
      ...
      setTimeout(function () {
        ...
        callBack()
        console.log(`You have arrived at ${gumShoe.city}! Thanks for flying skyblue.`)
      }, 2000)
